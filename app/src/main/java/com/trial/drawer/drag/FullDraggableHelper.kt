    ...
    private fun canNestedViewScroll(view: View , checkSelf: Boolean , dx: Int , x: Int , y: Int): Boolean {
        if (view is ViewGroup) {
            ...
            for (i in count - 1 downTo 0) {
                ...
                if (x + scrollX >= child.left && x + scrollX < child.right && y + scrollY >= child.top && y + scrollY < child.bottom && canNestedViewScroll(child , true , dx , x + scrollX - child.left , y + scrollY - child.top)) {
                    return true
                }
            }
        }
